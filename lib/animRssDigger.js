	  ...
	  while(item = stream.read()){
		//var obj = new Object();
		  ...
		  if(item.title.match(subTeam) && item.title.match(title) && item.title.match(epNo)){
            if(tempEp < 9){ //10 will pass one more to higher pos. Assume the episode numbers are including two digits
              nextEp = epNo.substr(0 , 1) + (1 + tempEp);
            }
            else{
              nextEp = (parseInt(epNo.substr(0 , 1)) + 1).toString() + (tempEp + 1).toString().substr(this.length - 1 , 1);
            }
